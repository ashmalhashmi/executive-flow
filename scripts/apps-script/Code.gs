...
/** Old full-replace tabs used Sr# — clear data and rewrite with Record ID rows. */
...
/** Build map Record ID → sheet row number (1-based). */
...
/**
 * appendRow for new Record IDs; update existing row if ID already in sheet.
 * idColIndex = 0-based column index of Record ID in each data row.
 */
...
/** Expenditure: summary rows 1–3, log header row 5, appendRow from row 6+. */
